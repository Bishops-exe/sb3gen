...
export function GetTranslate(words: Val, language: string | InputVal): Block | CompoundBlock {
  ...
  if (typeof language === 'string') {
    ...
    compound.main.addInput('WORDS', c(words));
    return compound;
  }
  return Block.create('translate_getTranslate').addInput('WORDS', c(words)).addInput('LANGUAGE', language);
}
...
export function LanguagesMenu(languages: string): Block {
  ...
  return Block.create('translate_menu_languages').addField('languages', languages);
}
